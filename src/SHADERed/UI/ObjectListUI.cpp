	...
	void ObjectListUI::Update(float delta)
	{
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (ImGui::BeginPopupContextItem(std::string("##context" + items[i]->Name).c_str())) {
				...
				if (oItem->Type == ObjectType::RenderTexture || oItem->Type == ObjectType::Image || 
					oItem->Type == ObjectType::Texture || oItem->Type == ObjectType::Texture3D || oItem->Type == ObjectType::CubeMap ||
					isImg3D || (isPluginOwner && pobj->Owner->Object_HasProperties(pobj->Type))) {
